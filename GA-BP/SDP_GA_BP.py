...
def train_bp_model(X_train, y_train, X_val, y_val, epochs=700, batch_size=16, l2_reg=0.001):
...
def optimize_parameters(population_size=50, generations=10):
...
def cross_validate(X, y, n_splits=3):
    ...
    for train_index, val_index in kf.split(X):
        ...
        train_ga_bp_model(bp_model, X_train, y_train, X_val, y_val, 20, 10)
...
if __name__ == "__main__":
    ...
    performance_metrics = evaluate_final_model(bp_model, X_test, y_test)
    print(f"Test set performance metrics (after adjustment): MSE={performance_metrics[0]}, MAE={performance_metrics[1]}, R2={performance_metrics[2]}")

    print("[*]Saving Model...")
    bp_model.save("SDP_GA-BP.h5")
