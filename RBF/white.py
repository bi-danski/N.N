...
class TestRBFModel(unittest.TestCase):
    ...
    def setUp(self):
        self.input_shape = 5  # Reduced from 10 to 5
        self.units = 8  # Reduced from 16 to 8
        self.gamma = 0.5
        self.X_train = np.random.rand(20, self.input_shape)  # Reduced from 100 to 20
        self.y_train = np.random.rand(20, 1)  # Reduced from 100 to 20
        self.X_test = np.random.rand(5, self.input_shape)  # Reduced from 20 to 5
        self.y_test = np.random.rand(5, 1)  # Reduced from 20 to 5
    ...
    def test_create_rbf_model(self):
        ...
        self.assertIsInstance(model.layers[1], RBFLayer)
        print("[+] Test Complete")
    ...
    def test_model_compile_and_fit(self):
        ...
        model.fit(self.X_train_scaled, self.y_train, epochs=1, batch_size=2, verbose=0)  # Reduced epochs and batch size
